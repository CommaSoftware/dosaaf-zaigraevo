...
// Courses Slider
const coursesWrap = document.querySelector('#courses .secrion__scroll-wrapper');
const coursesArr = document.querySelectorAll('#courses .courses')
if (!!coursesWrap && !!coursesArr.length) {
	let prevWrap = coursesWrap.closest('#courses').previousElementSibling;
	let nextWrap = coursesWrap.closest('#courses').nextElementSibling;
	...
	for(wheelEvent of wheelEvents) {
		...
		coursesWrap.addEventListener(wheelEvent, (e)=> {
			e.preventDefault()
			let current = document.querySelector('#courses .courses.is--active');
